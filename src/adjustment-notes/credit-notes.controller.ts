...
export class CreditNotesController {
  ...
  @ApiCreatedResponse({ type: CreatedDocumentResponseDto, description: 'Credit note registered with status PENDING.' })
  async create(@Body() input: CreateAdjustmentNoteDto, @CurrentCompanyId() companyId?: string): Promise<CreatedDocumentResponseDto> {
    return toCreatedDocumentResponse(await this.notes.create(input, companyId, DocumentType.CREDIT_NOTE));
  }
}
